...
void main() {
  group('Tennis game test', () {
    ...
    test('Tennis game => fortyZero', () {
      expect(fortyZero.getScore(), Score.points(PlayerPoints.forty(), PlayerPoints.zero()).getScore());
    }, skip: false);

    /* fortyThirty */
    Score fortyThirty = _updateAndPrintScore(TennisGame.scorePoint(fortyZero, Player.two()), Player.two());

    test('Tennis game => fortyThirty', () {
      expect(
        fortyThirty.getScore(),
        Score.points(PlayerPoints.forty(), PlayerPoints.thirty()).getScore(),
      );
    }, skip: false);
    ...
    test('Tennis game => deuce1', () {
      expect(deuce1.getScore(), Score.deuce().getScore());
    }, skip: false);
    ...
    test('Tennis game => advantageP1', () {
      expect(advantageP1.getScore(), Score.advantage(Advantage.one()).getScore());
    }, skip: false);
    ...
    test('Tennis game => gameP1', () {
      expect(gameP1.getScore(), Score.game(Game.one()).getScore());
    }, skip: false);
    ...
    test('Tennis game => zeroFifteen', () {
      expect(zeroFifteen.getScore(), Score.points(PlayerPoints.zero(), PlayerPoints.fifteen()).getScore());
    }, skip: false);
    ...
    test('Tennis game => deuce2', () {
      expect(deuce2.getScore(), Score.deuce().getScore());
    }, skip: false);
  });
}
...
Score _updateAndPrintScore(Score oldScore, Player player) {
  ...
  print(Score.getString(oldScore) + " + " + Player.getString(player) + " -> " + Score.getString(newScore));
  return newScore;
}
